...
{{Section 5.2 of RFC6750}}, for example,
...
OAuth scope, from {{Section 3.3 of RFC6749}}, is sometimes overloaded to convey the location or identity
...
in a Token Introspection {{RFC7662}} response. The authorization server may use
...
via the implicit flow ({{Section 4.2 of RFC6749}}), the requested resource is applicable
to that access token. In the code flow ({{Section 4.1 of RFC6749}}) where an
...
`https://api.example.com/app/` is shown in {{authz-endpoint-example-token}} below
...
Below in {{authz-endpoint-example-code}} is an example of an authorization request
...
across different services.  As specified in {{Section 5.1 of RFC6749}}, the authorization server must
...
Following from the code flow authorization request shown in {{authz-endpoint-example-code}},
the below examples show an `authorization_code` grant type access token request ({{token-endpoint-example-ac}})
and response ({{response-example-ac}}) where the client tells the authorization server that
...
`https://contacts.example.com/` is shown in {{token-endpoint-example-rt}} below
with the response shown in {{response-example-rt}}
(extra line breaks and indentation are for display purposes only).
